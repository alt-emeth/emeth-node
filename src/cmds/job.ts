...
> = {
  ...
  handler: async (args) => {
    ...
    const job = await emethCore.jobs(jobId);
    const jobAssign = await emethCore.jobAssigns(jobId);
    const jobDetail = await emethCore.jobDetails(jobId);
